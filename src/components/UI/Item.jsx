...
const Item = ({ itemInfo }) => {
  ...
  const makeFavorite = () => {
    ...
    for (let i = 0; i < favoritedMovies.length; i++) {
      if (favoritedMovies[i].title === title) {
        favoritedMovies = favoritedMovies.filter(
          (movie) => movie.title !== title
        );
        setIsFavorited((prev) => (prev = false));
        localStorage.setItem("favorites", JSON.stringify(favoritedMovies));
        return;
      }
    }
    favoritedMovies.push(itemInfo); // Add the new movie to the array
    setIsFavorited((prev) => (prev = true));
    localStorage.setItem("favorites", JSON.stringify(favoritedMovies));
  };
      ...
      <div className="movie__item-info">
        ...
        {category === "Movie" ? (
          <>
            ...
            <span className="movie__item-category"> {category} ‧</span>
          </>
        ) : (
          <>
            {" "}
